#! /bin/bash
QE_path='/usr/local/Quantum_Espresso_6.5/bin/'; #'/usr/QE/qe-6.7/bin' # Path on the Danymede machine

# # step 0: run scf to have the converged charge density
# mpirun -np 20 $QE_path/pw.x < RUN_scf_pw_QE.in > MoS2.scf.out

# # step1_Phonon_QE to have .dyn matrices (the most expensive step: should use many cores with this step)
# mpirun -np 20 $QE_path/ph.x < RUN_step1_Dyn_on_q_ph_QE.in > print_step1_phonon.out
