...
cd('/media/tranvt/Importance/DATA/RESEARCH/Works in time/2012-2015.In France.PhD/Dropbox/My papers/Drafts/2024.Paper2_DFT_phonon_transport/Matlab_codes_and_DFT_data/Some_DFT_data/MoS2-monolayer/NEW_with_vc_relax_forces1Em4/phonon_Nq_odd/Result') 
...
number_of_individual_files=9; % number of file .dyn1, .dyn2, ...

filename_dynmat_with_index=sprintf('MoS2.dyn');
...
alat_au=  5.9830; % a.u unit of length
alat=alat_au*au_of_length_to_Angstrom
Lattice_vector=alat*[1.0081  0.0000  0.0000
                     -0.5040  0.8729  0.0000
                     0.0000  0.0000  3.5653];
...
for i=2:npath
ln(i-1)=line([q_distance(Labels_position(i))-1 q_distance(Labels_position(i))-1]/(Nq_path-1), [-100 1.1*max(max(Freq))],'LineStyle','--');
end
